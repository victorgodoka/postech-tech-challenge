...
export const initializeAuth = createAsyncThunk(
  ...
  async () => {
    ...
    console.log('Sessão localStorage:', localSession);
    
    if (localSession && localSession.expiresAt > Date.now()) {
      console.log('Redux: Sessão válida encontrada no localStorage');
      return localSession;
    }
    ...
    try {
      console.log('Verificando usuários no IndexedDB para compatibilidade...');
    ...
    console.log('=== FIM INITIALIZE AUTH - NENHUMA SESSÃO ENCONTRADA ===');
    return null;
  }
);
